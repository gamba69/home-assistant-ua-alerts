...
def test_device_page_test_dialog_contract():
    ...
    assert 'label: stringsFor(hass).action' in js_source
    assert "`${stringsFor(hass).action} · ${territoryTitle}`" not in js_source
    assert 'customElements.whenDefined("ha-device-info-card")' in js_source
    assert 'querySelector(".manuf")' in js_source
    assert 'territory.className = "extra-info ua-alerts-territory"' in js_source
    assert 'territory.textContent = device.name || ""' in js_source
    assert 'manufacturer.insertAdjacentElement("afterend", territory)' in js_source
    assert 'querySelector("ha-device-info-card")' in js_source
    assert 'base.label.endsWith(territorySuffix)' in js_source
